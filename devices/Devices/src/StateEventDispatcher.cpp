...
StateEventDispatcher::StateEventDispatcher(StateRemoteObject* pRemoteObject, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_pRemoteObject(pRemoteObject)
{
	_pRemoteObject->stateChanged += Poco::delegate(this, &StateEventDispatcher::event__stateChanged);
	_pRemoteObject->statusChanged += Poco::delegate(this, &StateEventDispatcher::event__statusChanged);
}


StateEventDispatcher::~StateEventDispatcher()
{
	try
	{
		_pRemoteObject->stateChanged -= Poco::delegate(this, &StateEventDispatcher::event__stateChanged);
...
void StateEventDispatcher::event__stateChanged(const void* pSender, const Poco::UInt32& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void StateEventDispatcher::event__statusChanged(const void* pSender, const IoT::Devices::DeviceStatusChange& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void StateEventDispatcher::event__stateChangedImpl(const std::string& subscriberURI, const Poco::UInt32& data)
{
...
void StateEventDispatcher::event__statusChangedImpl(const std::string& subscriberURI, const IoT::Devices::DeviceStatusChange& data)
{
	remoting__staticInitBegin(REMOTING__NAMES);
...
const std::string StateEventDispatcher::DEFAULT_NS("");
} // namespace Devices
} // namespace IoT
