...
DatapointEventDispatcher::DatapointEventDispatcher(IDatapoint* pInterface, const Poco::RemotingNG::Identifiable::ObjectId& objectId, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_objectId(objectId),
	_pInterface(pInterface)
{
	_pInterface->invalidated += Poco::delegate(this, &DatapointEventDispatcher::event__invalidated);
	_pInterface->statusChanged += Poco::delegate(this, &DatapointEventDispatcher::event__statusChanged);
}


DatapointEventDispatcher::~DatapointEventDispatcher()
{
	try
	{
		_pInterface->invalidated -= Poco::delegate(this, &DatapointEventDispatcher::event__invalidated);
...
void DatapointEventDispatcher::event__invalidated(const void* pSender)
{
	if (pSender)
	{
		Poco::Clock now;
...
void DatapointEventDispatcher::event__statusChanged(const void* pSender, const IoT::Devices::DeviceStatusChange& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void DatapointEventDispatcher::event__invalidatedImpl(const std::string& subscriberURI)
{
...
void DatapointEventDispatcher::event__statusChangedImpl(const std::string& subscriberURI, const IoT::Devices::DeviceStatusChange& data)
{
	using namespace std::string_literals;
...
const std::string DatapointEventDispatcher::DEFAULT_NS("");
} // namespace Devices
} // namespace IoT
