...
CounterEventDispatcher::CounterEventDispatcher(CounterRemoteObject* pRemoteObject, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_pRemoteObject(pRemoteObject)
{
	_pRemoteObject->countChanged += Poco::delegate(this, &CounterEventDispatcher::event__countChanged);
	_pRemoteObject->statusChanged += Poco::delegate(this, &CounterEventDispatcher::event__statusChanged);
}


CounterEventDispatcher::~CounterEventDispatcher()
{
	try
	{
		_pRemoteObject->countChanged -= Poco::delegate(this, &CounterEventDispatcher::event__countChanged);
...
void CounterEventDispatcher::event__countChanged(const void* pSender, const Poco::Int32& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void CounterEventDispatcher::event__statusChanged(const void* pSender, const IoT::Devices::DeviceStatusChange& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void CounterEventDispatcher::event__countChangedImpl(const std::string& subscriberURI, const Poco::Int32& data)
{
...
void CounterEventDispatcher::event__statusChangedImpl(const std::string& subscriberURI, const IoT::Devices::DeviceStatusChange& data)
{
	using namespace std::string_literals;
...
const std::string CounterEventDispatcher::DEFAULT_NS("");
} // namespace Devices
} // namespace IoT
