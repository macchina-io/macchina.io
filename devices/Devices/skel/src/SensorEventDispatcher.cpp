...
SensorEventDispatcher::SensorEventDispatcher(ISensor* pInterface, const Poco::RemotingNG::Identifiable::ObjectId& objectId, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_objectId(objectId),
	_pInterface(pInterface)
{
	_pInterface->valueChanged += Poco::delegate(this, &SensorEventDispatcher::event__valueChanged);
	_pInterface->statusChanged += Poco::delegate(this, &SensorEventDispatcher::event__statusChanged);
}


SensorEventDispatcher::~SensorEventDispatcher()
{
	try
	{
		_pInterface->valueChanged -= Poco::delegate(this, &SensorEventDispatcher::event__valueChanged);
...
void SensorEventDispatcher::event__statusChanged(const void* pSender, const IoT::Devices::DeviceStatusChange& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
const std::string SensorEventDispatcher::DEFAULT_NS("");
} // namespace Devices
} // namespace IoT
