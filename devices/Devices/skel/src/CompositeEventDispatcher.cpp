...
CompositeEventDispatcher::CompositeEventDispatcher(IComposite* pInterface, const Poco::RemotingNG::Identifiable::ObjectId& objectId, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_objectId(objectId),
	_pInterface(pInterface)
{
	_pInterface->statusChanged += Poco::delegate(this, &CompositeEventDispatcher::event__statusChanged);
}
...
void CompositeEventDispatcher::event__statusChanged(const void* pSender, const IoT::Devices::DeviceStatusChange& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void CompositeEventDispatcher::event__statusChangedImpl(const std::string& subscriberURI, const IoT::Devices::DeviceStatusChange& data)
{
...
const std::string CompositeEventDispatcher::DEFAULT_NS("");
} // namespace Devices
} // namespace IoT
