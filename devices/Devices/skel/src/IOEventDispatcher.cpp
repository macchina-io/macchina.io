...
IOEventDispatcher::IOEventDispatcher(IIO* pInterface, const Poco::RemotingNG::Identifiable::ObjectId& objectId, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_objectId(objectId),
	_pInterface(pInterface)
{
	_pInterface->stateChanged += Poco::delegate(this, &IOEventDispatcher::event__stateChanged);
	_pInterface->statusChanged += Poco::delegate(this, &IOEventDispatcher::event__statusChanged);
}
...
void IOEventDispatcher::event__statusChanged(const void* pSender, const IoT::Devices::DeviceStatusChange& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void IOEventDispatcher::event__statusChangedImpl(const std::string& subscriberURI, const IoT::Devices::DeviceStatusChange& data)
{
	using namespace std::string_literals;
