...
GyroscopeEventDispatcher::GyroscopeEventDispatcher(IGyroscope* pInterface, const Poco::RemotingNG::Identifiable::ObjectId& objectId, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_objectId(objectId),
	_pInterface(pInterface)
{
	_pInterface->rotationChanged += Poco::delegate(this, &GyroscopeEventDispatcher::event__rotationChanged);
	_pInterface->statusChanged += Poco::delegate(this, &GyroscopeEventDispatcher::event__statusChanged);
}
...
void GyroscopeEventDispatcher::event__rotationChangedImpl(const std::string& subscriberURI, const IoT::Devices::Rotation& data)
{
	...
	static const std::string REMOTING__NAMES[] = {"rotationChanged"s,"subscriberURI"s,"data"s};
