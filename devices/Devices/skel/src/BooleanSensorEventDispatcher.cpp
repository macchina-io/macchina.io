//
// BooleanSensorEventDispatcher.cpp
//
// Library: IoT/Devices
// Package: Generated
...
BooleanSensorEventDispatcher::BooleanSensorEventDispatcher(IBooleanSensor* pInterface, const Poco::RemotingNG::Identifiable::ObjectId& objectId, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_objectId(objectId),
	_pInterface(pInterface)
{
	_pInterface->stateChanged += Poco::delegate(this, &BooleanSensorEventDispatcher::event__stateChanged);
	_pInterface->statusChanged += Poco::delegate(this, &BooleanSensorEventDispatcher::event__statusChanged);
}


BooleanSensorEventDispatcher::~BooleanSensorEventDispatcher()
{
	try
	{
		_pInterface->stateChanged -= Poco::delegate(this, &BooleanSensorEventDispatcher::event__stateChanged);
...
void BooleanSensorEventDispatcher::event__stateChanged(const void* pSender, const bool& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void BooleanSensorEventDispatcher::event__statusChanged(const void* pSender, const IoT::Devices::DeviceStatusChange& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void BooleanSensorEventDispatcher::event__stateChangedImpl(const std::string& subscriberURI, const bool& data)
{
...
void BooleanSensorEventDispatcher::event__statusChangedImpl(const std::string& subscriberURI, const IoT::Devices::DeviceStatusChange& data)
{
	using namespace std::string_literals;
...
const std::string BooleanSensorEventDispatcher::DEFAULT_NS("");
} // namespace Devices
} // namespace IoT
