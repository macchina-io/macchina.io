//
// SerialDeviceEventDispatcher.cpp
//
// Library: IoT/Devices
// Package: Generated
...
SerialDeviceEventDispatcher::SerialDeviceEventDispatcher(ISerialDevice* pInterface, const Poco::RemotingNG::Identifiable::ObjectId& objectId, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_objectId(objectId),
	_pInterface(pInterface)
{
	_pInterface->lineReceived += Poco::delegate(this, &SerialDeviceEventDispatcher::event__lineReceived);
	_pInterface->statusChanged += Poco::delegate(this, &SerialDeviceEventDispatcher::event__statusChanged);
}


SerialDeviceEventDispatcher::~SerialDeviceEventDispatcher()
{
	try
	{
		_pInterface->lineReceived -= Poco::delegate(this, &SerialDeviceEventDispatcher::event__lineReceived);
...
void SerialDeviceEventDispatcher::event__lineReceived(const void* pSender, const std::string& data)
{
	if (pSender)
	{
		Poco::Clock now;
		...
		while (it != _subscribers.end())
		{
			...
			else
			{
				try
				{
					event__lineReceivedImpl(it->first, data);
				}
...
void SerialDeviceEventDispatcher::event__statusChanged(const void* pSender, const IoT::Devices::DeviceStatusChange& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void SerialDeviceEventDispatcher::event__lineReceivedImpl(const std::string& subscriberURI, const std::string& data)
{
	using namespace std::string_literals;
	
	static const std::string REMOTING__NAMES[] = {"lineReceived"s,"subscriberURI"s,"data"s};
...
void SerialDeviceEventDispatcher::event__statusChangedImpl(const std::string& subscriberURI, const IoT::Devices::DeviceStatusChange& data)
{
	using namespace std::string_literals;
...
const std::string SerialDeviceEventDispatcher::DEFAULT_NS("");
} // namespace Devices
} // namespace IoT
