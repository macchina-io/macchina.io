...
LEDEventDispatcher::LEDEventDispatcher(ILED* pInterface, const Poco::RemotingNG::Identifiable::ObjectId& objectId, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_objectId(objectId),
	_pInterface(pInterface)
{
	_pInterface->statusChanged += Poco::delegate(this, &LEDEventDispatcher::event__statusChanged);
}


LEDEventDispatcher::~LEDEventDispatcher()
{
	try
	{
		_pInterface->statusChanged -= Poco::delegate(this, &LEDEventDispatcher::event__statusChanged);
...
void LEDEventDispatcher::event__statusChanged(const void* pSender, const IoT::Devices::DeviceStatusChange& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void LEDEventDispatcher::event__statusChangedImpl(const std::string& subscriberURI, const IoT::Devices::DeviceStatusChange& data)
{
...
const std::string LEDEventDispatcher::DEFAULT_NS("");
} // namespace Devices
} // namespace IoT
