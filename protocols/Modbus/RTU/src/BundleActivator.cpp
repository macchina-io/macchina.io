...
public:
	...
	void start(BundleContext::Ptr pContext)
	{
		...
		for (std::vector<std::string>::const_iterator it = keys.begin(); it != keys.end(); ++it)
		{
			...
			{
				const std::string device = _pPrefs->configuration()->getString(baseKey + ".device"s, ""s);
				const std::string params = _pPrefs->configuration()->getString(baseKey + ".params"s, "8N1"s);
