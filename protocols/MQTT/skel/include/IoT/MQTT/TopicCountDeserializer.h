...
public:
	...
	static void deserializeImpl(Deserializer& deser, IoT::MQTT::TopicCount& value)
	{
		...
		static const std::string REMOTING__NAMES[] = {"topic"s,"messageCount"s,"lastMessage"s};
		TypeDeserializer<std::string>::deserialize(REMOTING__NAMES[0], true, deser, value.topic);
		TypeDeserializer<int>::deserialize(REMOTING__NAMES[1], true, deser, value.messageCount);
		TypeDeserializer<Poco::Timestamp>::deserialize(REMOTING__NAMES[2], true, deser, value.lastMessage);
	}
