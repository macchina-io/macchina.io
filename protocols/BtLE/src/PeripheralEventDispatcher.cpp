...
#include "IoT/BtLE/IndicationDeserializer.h"
#include "IoT/BtLE/IndicationSerializer.h"
#include "IoT/BtLE/NotificationDeserializer.h"
#include "IoT/BtLE/NotificationSerializer.h"
...
PeripheralEventDispatcher::~PeripheralEventDispatcher()
{
	try
	{
		_pRemoteObject->connected -= Poco::delegate(this, &PeripheralEventDispatcher::event__connected);
...
void PeripheralEventDispatcher::event__connected(const void* pSender)
{
	if (pSender)
	{
		Poco::Clock now;
...
void PeripheralEventDispatcher::event__disconnected(const void* pSender)
{
	if (pSender)
	{
		Poco::Clock now;
...
void PeripheralEventDispatcher::event__error(const void* pSender, const std::string& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void PeripheralEventDispatcher::event__indicationReceived(const void* pSender, const IoT::BtLE::Indication& data)
{
	if (pSender)
	{
		Poco::Clock now;
		...
		while (it != _subscribers.end())
		{
			...
			else
			{
				try
				{
					event__indicationReceivedImpl(it->first, data);
				}
				catch (Poco::RemotingNG::RemoteException&)
				{
					throw;
...
void PeripheralEventDispatcher::event__notificationReceived(const void* pSender, const IoT::BtLE::Notification& data)
{
	if (pSender)
	{
		Poco::Clock now;
		...
		while (it != _subscribers.end())
		{
			...
			else
			{
				try
				{
					event__notificationReceivedImpl(it->first, data);
				}
				catch (Poco::RemotingNG::RemoteException&)
				{
					throw;
...
const std::string PeripheralEventDispatcher::DEFAULT_NS("");
} // namespace BtLE
} // namespace IoT
