...
public:
	...
	static void serializeImpl(const IoT::DeviceStatus::DeviceStatusChange& value, Serializer& ser)
	{
		...
		static const std::string REMOTING__NAMES[] = {"message"s,"currentStatus"s,"previousStatus"s,""s};
		TypeSerializer<Poco::Optional<IoT::DeviceStatus::StatusMessage>>::serialize(REMOTING__NAMES[0], value.message, ser);
		TypeSerializer<int>::serialize(REMOTING__NAMES[1], static_cast<int>(value.currentStatus), ser);
