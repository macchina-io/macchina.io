...
public:
	...
	static void serializeImpl(const IoT::DeviceStatus::StatusMessage& value, Serializer& ser)
	{
		...
		TypeSerializer<std::string >::serialize(REMOTING__NAMES[3], value.source, ser);
		TypeSerializer<int >::serialize(REMOTING__NAMES[4], value.status, ser);
		TypeSerializer<std::string >::serialize(REMOTING__NAMES[5], value.text, ser);
		TypeSerializer<Poco::DateTime >::serialize(REMOTING__NAMES[6], value.timestamp, ser);
	}
