...
// ClientWrapper.cpp
//
// $Id: //poco/1.4/JS/OPC/src/ClientWrapper.cpp#6 $
//
// Library: JS/OPC
// Package: Wrappers
...
{
	std::string server;
	int port = Poco::OPC::OPC_STANDARD_PORT;
	std::string user;
	std::string pass;
	bool doConnect = true;
	...
	try
	{
		...
		if(pClientHolder)
		{
			if (args.Length() >= 2)
			{
				...
				if (ti == typeid(Poco::OPC::DateTime))
				{
					v8::Local<v8::String> ret = v8::String::NewFromUtf8(v8::Isolate::GetCurrent(),
						val.convert<Poco::OPC::DateTime>().toString().c_str());
				...
				else if (val.isNumeric())
				{
					if (val.isInteger())
					{
						...
						if (val.isSigned())
						{
							ret = v8::Integer::New(v8::Isolate::GetCurrent(), val.convert<Int64>());
						}
						else
						{
							ret = v8::Integer::New(v8::Isolate::GetCurrent(), val.convert<UInt64>());
						}
						args.GetReturnValue().Set(ret); return;
					}
					else // floating point
